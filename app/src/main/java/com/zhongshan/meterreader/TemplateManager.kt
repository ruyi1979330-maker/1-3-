...
object TemplateManager {
    ...
    // 设备列表 (精简：仅保留特灵与板交 + 本次新增约克)
    // 红线：约克 pumpFieldIds 显式赋值为 emptyList()，isHeatExchanger 为 false。
    val allTemplates = listOf(
        ...
        // 【本次新增】3号机房 约克螺杆机 1#/2#（同型号，归 3 号机房表单）
        DeviceTemplate("3号机房 约克螺杆1#", "york_1",    3, URL_ROOM_3, pumpFieldIds = emptyList(), isHeatExchanger = false),
        DeviceTemplate("3号机房 约克螺杆2#", "york_2",    3, URL_ROOM_3, pumpFieldIds = emptyList(), isHeatExchanger = false),
    ...
    fun getTabName(template: DeviceTemplate): String = when {
        ...
        // 【本次新增】约克机组需切换至专属 Tab「螺杆机组（约克）」
        template.machineId.startsWith("york")                   -> "螺杆机组（约克）"
        else                                                    -> "螺杆机组"
    }
}
