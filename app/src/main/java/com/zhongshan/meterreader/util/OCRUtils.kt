...
object OCRUtils {
    ...
    suspend fun recognizeFieldsParallel(
            ...
            async {
                ocrSemaphore.withPermit {
                    ...
                    try {
                        ...
                        canvas.drawBitmap(sourceBitmap, rect,
                            Rect(0, 0, rect.width(), rect.height()), REUSABLE_PAINT)
                        val image = InputImage.fromBitmap(pooledBitmap, 0)
                        val visionText = recognizer.process(image).await()
                        val cleanedText = cleanOcrText(visionText.text)
                        if (cleanedText.isNotEmpty()) outData[fieldId] = cleanedText
                    } catch (e: Exception) {
                        e.printStackTrace()
                    } finally {
        ...
        while (bitmapPool.isNotEmpty()) bitmapPool.poll()?.recycle()
        return@withContext outData
    }

    // =====================================================================
    // 板交整图识别
    // Bug Fix：关键字按长度降序排列，优先精确匹配；
    // 每次命中新关键字立即更新前缀，防止多板交数据串号。
    // =====================================================================
