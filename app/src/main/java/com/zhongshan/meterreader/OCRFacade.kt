...
object OCRFacade {
    ...
    private suspend fun extractYorkDataFromBitmap(bitmap: Bitmap, tag: String): Map<String, String> = withContext(Dispatchers.IO) {
        ...
        // "6"（真实值是"68"，丢了一位），这种半个数字如果直接采信、乘以2.5倍系数后
        // 会变成一个"看着还算合理但其实完全错误"的数（15.0 而非真实的170.0），
            ...
            if (anchorYForRetry == null) {
                ...
                try {
                    ...
                    if (cropW > 4 && cropH > 4) {
                        ...
                        DebugLogger.log(tag, "满载安培-二次探测(第一级): 裁剪(left=$left top=$top w=$cropW h=$cropH) 放大${scale1}倍后=${level1Zoom.width}x${level1Zoom.height}")
                        ...
                        level1Lines.forEach {
                            ...
                            DebugLogger.log(tag, "满载安培-第一级识别行: Text=${it.text} Box=${bx?.left},${bx?.top},${bx?.right},${bx?.bottom}")
                        }

                        fun plausibleNum(texts: List<String>): String? =
                            texts.flatMap { t -> Regex("""-?\d{1,4}(\.\d{1,2})?""").findAll(t).map { m -> m.value } }
                                .firstOrNull { n -> !n.contains(".") && n.length >= 2 && (n.toIntOrNull() ?: -1) in 0..100 }
                        ...
                        if (box != null) {
                            ...
                            if (w2 > 4 && h2 > 4) {
                                ...
                                DebugLogger.log(tag, "满载安培-二次探测(第二级): 围绕'${motorLine.text}'再裁剪并放大${scale2}倍后=${level2Zoom.width}x${level2Zoom.height}")
                        ...
                        if (finalNum != null) {
                            DebugLogger.log(tag, "满载安培-二次探测命中: 值=$finalNum")
                    ...
                    try { level1Crop?.let { if (!it.isRecycled) it.recycle() } } catch (_: Throwable) {}
                    try { level1Zoom?.let { if (!it.isRecycled) it.recycle() } } catch (_: Throwable) {}
                    try { level2Crop?.let { if (!it.isRecycled) it.recycle() } } catch (_: Throwable) {}
                    try { level2Zoom?.let { if (!it.isRecycled) it.recycle() } } catch (_: Throwable) {}
                }
            }
        }
