...
class WebViewActivity : AppCompatActivity() {
    ...
    private fun compileFillJs(
        ...
        (function(){
            ...
            // 勾选冷冻泵复选框 (引入防抖机制，防止异步渲染导致的重复点击取消)
            ...
            function checkPump(pumpName) {
                ...
                for (var i = 0; i < labels.length; i++) {
                    ...
                    if (cleanText(title) === pumpClean) {
                        var isChecked = labels[i].querySelector('.Checkbox-checked') || labels[i].className.indexOf('Checkbox-checked') > -1;
                        if (isChecked) return 2;
                        
                        var now = Date.now();
                        var lastTime = pumpsLastClickTime[pumpName] || 0;
                        // 2秒内不重复点击同一个泵，等待 React 渲染完毕
