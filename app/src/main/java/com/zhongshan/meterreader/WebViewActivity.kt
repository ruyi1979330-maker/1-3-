    ...
    private fun compileFillJs(
        ...
        (function(){
            ...
            function setInputValue(formItem, value) {
                ...
                // 1. 数值输入框：原生setter赋值 + 完整事件链 (去除 click/focus 避免分组意外折叠)
            ...
            // 勾选冷冻泵复选框 (3秒冷却期 + 最大重试3次 + 成功后永久锁定，彻底解决异步渲染导致的反向取消)
            // 返回值：0=未找到/防抖中，1=本次新勾选，2=已勾选无需操作
            var pumpsState = {}; 
            var MAX_PUMP_ATTEMPTS = 3; // 最多尝试点击3次
            
            function checkPump(pumpName) {
                if (!pumpsState[pumpName]) {
                    pumpsState[pumpName] = { attempts: 0, confirmed: false, lastClickTime: 0 };
                }
                var state = pumpsState[pumpName];
                
                // 如果已经确认勾选成功，或者尝试次数耗尽，直接跳过，绝不再点击
                if (state.confirmed) return 2;
                if (state.attempts >= MAX_PUMP_ATTEMPTS) return 0;
                ...
                for (var i = 0; i < labels.length; i++) {
                    ...
                    if (cleanText(title) === pumpClean) {
                        ...
                        // 1. 精准判断是否已勾选 (优先看 label 和 span 的 class)
                        var isChecked = labels[i].className.indexOf('Checkbox-checked') > -1;
                        if (!isChecked) {
                        ...
                        if (isChecked) {
                            state.confirmed = true; // 永久确认，后续绝不触碰
                            return 2;
                        }
                        
                        // 2. 未勾选，检查是否可以点击
                        var now = Date.now();
                        // 首次点击立即执行；如果没确认，等待 3 秒再重试，给 React 充足的渲染时间
                        if (state.attempts === 0 || now - state.lastClickTime > 3000) {
                            state.attempts++;
            ...
            function scanAndAutofillEngine() {
                try {
                    ...
                    // 修复：只在首次或未找到元素时尝试展开分组，避免反复展开/收起导致 DOM 抖动遗漏
                    ...
                    // 收敛机制优化：连续3次未突破历史最大值则停止，彻底杜绝死循环
